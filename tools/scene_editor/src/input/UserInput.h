...
struct CursorMovement
{
    ...
    //
    // `position - offset` is the cursor position before this event.
    vec2 offset;
    ...
    /**
     * @brief Invert the y-axis of cursor coordinates.
     *
     * This effectively changes the cursor coordinate system origin to the
     * lower-left corner of the screen (default is upper-right).
     */
    constexpr auto invertY() const -> CursorMovement
    {
        return {
            .position{ position.x, areaSize.y - position.y },
...
struct UserInput
{
    ...
    UserInput(trc::Key key, trc::InputAction action);
    ...
    UserInput(trc::MouseButton button, trc::InputAction action);
