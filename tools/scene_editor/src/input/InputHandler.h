...
public:
    ...
    /**
     * @brief Bind a specific input event to a command.
     *
     * @param input   A specific input for which the command will be executed.
     * @param command A command. Will be executed each time an event occurs that
     *                matches `input`. Pass `nullptr` to clear any command that
     *                is currently set for this input.
     */
    ...
    /**
     * @brief Set a callback for key inputs that are not handled by any other
     *        command.
     */
    ...
    /**
     * @brief Set a callback for mouse inputs that are not handled by any other
     *        command.
     */
    ...
    /**
     * @brief Signal that this input frame should be exited.
     *
     * When the command execution which calls this function completes, the input
     * frame will be removed from the input stack and control will be passed
     * to the next frame on the stack. An exception is the root frame of the
     * input stack, which can never exit. In this case, any call to `exitFrame`
     * will be ignored.
     */
    void exitFrame();

    /**
     * @brief Test whether `exitFrame` was called on this input frame.
     */
...
public:
     ...
     * InvertibleActions and any pushed frame will be discarded.
     */
    void discardEvent();
     ...
     */
    auto keyboard() const -> const KeyboardState&;

    /**
     * @return The mouse state of the current viewport.
     */
    auto mouse() const -> const MouseState&;
...
private:
    ...
    {
        // The event was not handled by the command and may be passed on to
        // delegated services.
 ...
 */
class InputHandler : public EventTarget
{
public:
    /**
     * @brief Trivial execution strategy that executes actions immediately.
     */
    struct ImmediateActionExecutor : public ActionExecutor
    {
        void execute(u_ptr<InvertibleAction> action) {
            action->apply();
        }
    };
     ...
     *
     * @param rootFrame  Must not be `nullptr`.
     * @param exec       An action execution strategy. Defaults to a trivial
     *                   strategy that executes actions immediately and does not
     *                   keep track of a history. Must not be `nullptr`.
     *
     * @throw std::invalid_argument if any required argument is `nullptr`.
     */
    explicit
    InputHandler(u_ptr<InputFrame> rootFrame = std::make_unique<InputFrame>(),
                 s_ptr<ActionExecutor> exec = std::make_shared<ImmediateActionExecutor>());
    ...
    /**
     * @brief Set a new action executor.
     *
     * @param exec An action execution strategy. Must not be `nullptr`.
     *
     * @throw std::invalid_argument if `exec == nullptr`.
     */
    void setActionExecutor(s_ptr<ActionExecutor> exec);

private:
    auto processEvent(auto&& event, CommandExecutionContext& ctx) -> NotifyResult;
    ...
    s_ptr<ActionExecutor> actionExecutor;
};
