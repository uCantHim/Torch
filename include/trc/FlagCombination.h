...
namespace trc
{
    ...
    concept FlagBitsType = requires {
        std::is_enum_v<T>;
        // T::eNone;
        // // Requires that T::eNone is 0
        // std::same_as<
        //     std::integral_constant<std::underlying_type_t<T>,
        //                            static_cast<std::underlying_type_t<T>>(T::eNone)>,
        //     std::integral_constant<std::underlying_type_t<T>, 0>
        // >;
        T::eMaxEnum;
    };
    ...
    class FlagCombination
    {
        ...
        {
            values[I] = FlagValue{ 0, static_cast<ui32>(std::tuple_element_t<I, Tuple>::eMaxEnum) };
            if constexpr (I > 0) {
                _initElems<I - 1>();
            }
        }
    ...
    public:
        ...
        {
            values[staticIndex<T>].bits = static_cast<ui32>(flagBit);
        }
        ...
        constexpr auto toIndex() const -> ui32
        {
            ui32 index{ 0 };
            ui32 totalMax{ 1 };
    ...
    private:
        ...
        FlagValue values[std::tuple_size_v<Tuple>];
    };
