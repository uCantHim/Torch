...
TEST_F(AssetRegistryModuleStorageTest, HasModule)
{
    ...
    ASSERT_THROW(storage.get<trc::Geometry>(), std::out_of_range);
    ASSERT_THROW(storage.get<trc::Material>(), std::out_of_range);
    ASSERT_THROW(storage.get<trc::Texture>(), std::out_of_range);
    ASSERT_THROW(storage.get<DummyAsset>(), std::out_of_range);
    ...
    ASSERT_THROW(storage.get<trc::Material>(), std::out_of_range);
    ASSERT_THROW(storage.get<trc::Texture>(), std::out_of_range);
    ASSERT_NO_THROW(storage.get<DummyAsset>());
}
...
TEST_F(AssetRegistryModuleStorageTest, StoresCorrectObject)
{
    ...
    ASSERT_NE(dynamic_cast<DummyRegistry*>(&storage.get<DummyAsset>()), nullptr);
    ASSERT_EQ(mod, &storage.get<DummyAsset>());
    ASSERT_EQ(mod->getUpdateCounter(), 2);
    ASSERT_EQ(dynamic_cast<DummyRegistry*>(&storage.get<DummyAsset>())->getUpdateCounter(), 2);
}

TEST_F(AssetRegistryModuleStorageTest, Update)
{
    storage.addModule<DummyAsset>(std::make_unique<DummyRegistry>());
    DummyRegistry& mod = dynamic_cast<DummyRegistry&>(storage.get<DummyAsset>());
